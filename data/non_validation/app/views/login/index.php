  ...
  <div class="shadow-2xl max-w-md w-full mx-auto mt-4 bg-white p-8 border border-gray-300">
    <form method="POST" action="<?= BASEURL ?>/auth/login">
      <div class="mt-2">
        ...
        <input class="w-full p-2 border border-gray-300 rounded mt-1 duration-300 hover:shadow-xl transform hover:-translate-y-1 focus:-translate-y-1" type="text" name="email" id="email" value="<?= is_null($param) ? '' : $param['email'] ?>">
        <p id="err_username" class="text-red-600"><?= is_null($data) ? '' : $data['username'] ?></p>
      </div>
      <div class="mt-2">
        <label class="text-sm font-black text-gray-600 block" for="password">Password</label>
        <input class="w-full p-2 border border-gray-300 rounded mt-1 duration-300 hover:shadow-xl transform hover:-translate-y-1 focus:-translate-y-1" type="password" name="password" id="password" value="<?= is_null($param) ? '' : $param['password'] ?>">
        <p id="err_password" class="text-red-600"><?= is_null($data) ? '' : $data['password'] ?></p>
      </div>
      <div class="mt-3">
